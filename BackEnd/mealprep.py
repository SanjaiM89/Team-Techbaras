...
# Generate a meal plan for the selected day only
@router.post("/mealplans/generate/{day_index}")
async def generate_meal_plan_for_day(day_index: int, user_id: str = Depends(get_current_user)):
    logger.info(f"Generating meal plan for day_index: {day_index}, user_id: {user_id}")
    if day_index < 0 or day_index > 6:
        raise HTTPException(status_code=400, detail="Invalid day index")
    ...
    try:
        ...
        logger.debug("User details: height=%s cm, weight=%s kg, age=%s years, gender=%s, activity=%s, goals=%s, preferences=%s",height_cm, weight_kg, age, gender, activity_level, goals, preferences)
